...
4. Confirm the visible tagline says exactly: "Plan a better beach day with dog access and water-quality information."
5. Confirm the page, OpenGraph and Twitter descriptions all say exactly: "Plan a better beach day with dog access and water-quality information. Explore every Environment Agency-designated bathing water in England, plus carefully curated non-designated beaches."
6. Drive the filters (counts below are approximate; the EA dataset can
   drift a few beaches release to release; re-derive if `beachCount`
   in `data/beaches.json`'s `meta` differs meaningfully from 470):
   - Total: 464 EA-designated bathing waters + 6 explicitly curated
     non-designated extras = 470. This is not a list of every named beach in
     England.
   ...
   - Reset filters button → all controls (Sand, Region, County,
     ...
     count shown.
7. "Get directions" on a card: opens a new tab to
...
8. Data refresh: `python3 scripts/build_data.py` pages the whole EA
...
9. Site-hygiene checks:
   ...
   - Open the devtools console before and after loading the page:
     ...
     in `index.html`).
   - Footer shows two human-readable dates ("Dataset generated <current
     generated date> · 470 beaches" and "Dog rules last verified <oldest
